...
![image-20210528160026642](https://gitee.com/sheep-are-flying-in-the-sky/my-picture/raw/master/picture9/image-20210528160026642.png)

---

![image-20210528160524139](https://gitee.com/sheep-are-flying-in-the-sky/my-picture/raw/master/picture9/image-20210528160524139.png)

---

<img src="https://gitee.com/sheep-are-flying-in-the-sky/my-picture/raw/master/picture9/image-20210528160704346.png" alt="image-20210528160704346" style="zoom:50%;" />

---

![image-20210529180851105](https://gitee.com/sheep-are-flying-in-the-sky/my-picture/raw/master/picture9/image-20210529180851105.png)

---

![image-20210528162448386](https://gitee.com/sheep-are-flying-in-the-sky/my-picture/raw/master/picture9/image-20210528162448386.png)
...
<img src="https://gitee.com/sheep-are-flying-in-the-sky/my-picture/raw/master/picture9/image-20210529200038779.png" alt="image-20210529200038779" style="zoom: 33%;" />

---

<img src="https://gitee.com/sheep-are-flying-in-the-sky/my-picture/raw/master/picture9/image-20210529200112922.png" alt="image-20210529200112922" style="zoom:33%;" />
...
sbit RS = P3^0;				//1为数据，0命令
sbit RW = P3^1;				//1读, 0写
sbit E = P3^2;				//使能端
sbit mySwitch = P1^0;	//开关

unsigned char str1[] = {"Welcome!"};			
unsigned char str2[] = {"Keyuan Hunan"};
...
void initlcd(){
	writecom(0x38);		//设置：16*2显示， 5*7点阵, 8位数据接口
	writecom(0x0f);		//设置：开起显示， 显示光标
	writecom(0x06);		//写一个字符后， 地址指针（+1）
	writecom(0x01);		//显示清0， 数据指针清0
}

//显示字符串
void show_str(unsigned char address, unsigned char str[]){
	unsigned char i = 0;
	
	writecom(address);						//第一行位置：写命令（adress为命令地址）
	delay(5);
...
void fix_display(){
	...
	show_str(0x80 + 4, str1);					//第一行：显示 Welcome!
	show_str(0x80 + 2 + 0x40, str2);  //第二行：显示 Keyuan Hunan	
...
void float_display(){
	...
	writecom(0x01);		//显示清0， 数据指针清0
	writecom(0x06);
	show_str(0x80+0x0f, str1);						//第一行：显示 Welcome!
	show_str(0x80+0x40+0x0f, str2);  			//第二行：显示 Keyuan Hunan	

	while(1){
		//左移28 = 16 + keyuan Hunan(12)
		for(i = 0; i < 28; i++){  
				if(mySwitch)										//判断：如何开关置为1， 则跳出
					return;
				writecom(0x18);									//所有字符：向左移动1位
				delay(100);
		}
	
		//右移29 = 16 + keyuan Hunan(12)
		for(i = 0; i < 29; i++){
			if(mySwitch)											//判断：如何开关置为1， 则跳出
					return;
				writecom(0x1c);									//所有字符：向右移动1位
				delay(100);
		}
...
int main(void){
	...
	while(1){
		if(mySwitch){						//开关为高电平时：显示（固定字符）
				fix_display();
		}else{
				float_display();		//开关为低电平时：显示（浮动字符）
		}
	}
}
~~~
...
> 手机拨号：实现可以按下数字，可以删除， 可以清空



## 11.2、电路图

![image-20210603183355183](https://gitee.com/sheep-are-flying-in-the-sky/my-picture/raw/master/picture9/image-20210603183355183.png)
...
unsigned char str1[] = {"--phone number--"};			
unsigned char str2[16] = {"call:           "};


char flag = 1; //作为按键：添加到数组（防抖动）的标志位

char temp; //循环变量
char state = 1;  			//是否需要刷新

uchar num = 666;      //用于存放：当前按钮（按下）显示的0~9的地址索引



//显示的：电话数字（字符）
uchar code show_phone_num[]=
{
 //前三个数对应：电机（1， 2， 3数字），第四个数字（对应 - ）符号
 '1','2','3',0x40, 

 //前三个数对应：电机（4， 5， 6数字），第四个数字（未使用）
 '4','5','6',0x07,
 //前三个数对应：电机（7， 8， 9数字），第四个数字（未使用）
 '7','8','9',0x7c,
 //前三个数对应：电机（*， 0， #），第四个数字（未使用）
 '*','0','#',0x40
};
...
void display(){
	...
	show_str(0x80, str1);					//第一行：显示 "--phone number--"
	show_str(0x80 + 0x40, str2);  //第二行：显示 "call:           "
}

//将按下数字：存放入数组
void add_phone_array(){
 unsigned char i = 0;					//作为循环变量

	for(i = 5; i < 16; i++){			//call: 第5个索引就是（冒号）后面：第一个元素
	if(str2[i] == ' '){
			str2[i] = show_phone_num[num]; 			
			break;
	}
 }
}
...
void FunctionJudge(){
	  char i = 0;
		//如果按键在这个范围内：说明需要添加到（数组中）
		if((num <= 10 || num == 13) &&  flag == 1){
		add_phone_array(); 		//数字：添加到电话号码（数组中）
		flag = 0; 			//标志为：防止按键抖动
		state = 1;
 }else if(show_phone_num[num] == '*'){ //代表（*） 去除1个数字
		for(i = 15; i > 4; i--){
			if(str2[i] != ' '){
				str2[i] = ' ';
				break;
	  ...
	  state = 1;							//刷新一次：电话号码
	  num = 666;							//num 修改， 防止一直做重复操作
 }else if(show_phone_num[num] == '#'){ //代表（#）清空键 ， 清除所有电话数字
		for(i = 5; i < 16; i++){
			str2[i] = ' ';
		} 
	  state = 1;							//刷新一次：电话号码
...
void main(){
 ...
 while(1){	 
	 if(state){							//state初始为1， 代表需要刷新（电话号码）， 0为不刷新
			display();						
			state = 0;					//刷新一次后：置位0
		 }
	 
	 key_scan(); 						//检测按键是否：被按下 
	 FunctionJudge();				//对电子键盘按下的值， 做相应的判断操作， 做完操作后，会刷新一次（电话号码）
	}
}
