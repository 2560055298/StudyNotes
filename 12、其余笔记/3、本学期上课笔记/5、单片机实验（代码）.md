...
//用于存储：八位一体数码管（已按下的：最近的8个数字）
uchar storeNum[8] = {1, 2, 3, 4, 5, 6, 7, 8}; 
uchar count = 8;			//代表数组还能存的：数字索引下标(8代表可以存8个)， 0代表存满了
...
void test(){
	...
	//此时已按下：按键，将按下的按键（显示数字的索引：存入数组）
	if(count == 8){
		storeNum[7] = num;
		count--;
	}else if(count != 0){
		for(j = 7; j > 0; j++){
			storeNum[j-1] = storeNum[j];
			storeNum[7] = num;
			count--;
		}
	}else{
...
void show_differ(){
		...
		for(i = 0; i < 8; i++){
			...
			P3 = led[storeNum[i]];				//依次：显示数组中（索引对应的数字）
			delay(1);											//延迟
			P0 = 0xFF;										//清屏：所有都不选通
		}
}
